...
class WebpackProjectSniappet {
  ...
  getPath(resource) {
    ...
    for (const key in this.aliasMap) {
      ...
      try {
        // 当引入node_modules中方法时，特殊处理，引入方式修改
        const nodeModulePath = 'node_modules/'
        const nodeModuleLen = nodeModulePath.length
        let index = resource.indexOf(nodeModulePath)
        if (index > -1) {
          execPath = resource.slice(index + nodeModuleLen)
          findAliasPath = true;
          break;
        } else if (resource.startsWith(value)) {
